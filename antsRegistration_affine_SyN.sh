#!/bin/bash
# ARG_HELP([A wrapper around antsRegistration providing optimized registration pyramids])
...
# ARG_OPTIONAL_SINGLE([otsu-foreground-threshold],[],[Otsu class threshold which separates background from foreground when the object extent is measured without a mask, four Otsu thresholds give classes 1 to 5],[1.5])
...
# ARG_OPTIONAL_SINGLE([final-iterations-nonlinear],[],[Maximum iterations at finest scale for non-linear automatic generation],[20])
...
### START OF CODE GENERATED BY Argbash v2.12.0 one line above ###
...
_arg_otsu_foreground_threshold="1.5"
...
print_help()
{
  ...
  printf 'Usage: %s [-h|--help] [--moving-mask <arg>] [--fixed-mask <arg>] [--(no-)mask-all-linear] [--(no-)mask-extract] [--(no-)keep-mask-after-extract] [--otsu-foreground-threshold <arg>] [-o|--resampled-output <arg>] [--resampled-linear-output <arg>] [--initial-transform <arg>] [--linear-type <LINEAR>] [--(no-)close] [--(no-)rough] [--fixed <arg>] [--moving <arg>] [--weights <arg>] [--convergence <arg>] [--(no-)skip-linear] [--linear-metric <arg>] [--linear-shrink-factors <arg>] [--linear-smoothing-sigmas <arg>] [--linear-convergence <arg>] [--final-iterations-linear <arg>] [--(no-)kmeans-transformed-linear] [--(no-)skip-nonlinear] [--syn-control <arg>] [--syn-metric <arg>] [--syn-shrink-factors <arg>] [--syn-smoothing-sigmas <arg>] [--syn-convergence <arg>] [--final-iterations-nonlinear <arg>] [--(no-)histogram-matching] [--winsorize-image-intensities <arg>] [--(no-)reproducibility] [--random-seed <arg>] [--(no-)fast] [--(no-)float] [--(no-)float-linear] [--(no-)float-nonlinear] [-c|--(no-)clobber] [-v|--(no-)verbose] [-d|--(no-)debug] <movingfile> <fixedfile> <outputbasename>\n' "$(basename "$0")"
  ...
  do
    ...
    case "$_key" in
      ...
      --no-keep-mask-after-extract|--keep-mask-after-extract)
        ...
        ;;
      --otsu-foreground-threshold)
        test $# -lt 2 && die "Missing value for the optional argument '$_key'." 1
        _arg_otsu_foreground_threshold="$2"
        shift
        ;;
      --otsu-foreground-threshold=*)
        _arg_otsu_foreground_threshold="${_key##--otsu-foreground-threshold=}"
        ;;
      -o|--resampled-output)
        test $# -lt 2 && die "Missing value for the optional argument '$_key'." 1
...
object_extent() {
  ...
  if [[ -s "${mask}" ]]; then
    ...
    ThresholdImage 3 "${fg}" "${fg}" "${threshold}" Inf 1 0 >/dev/null
...
function make_syn_pyramid {
  ...
  while [[ $# -gt 0 ]]; do
    ...
    --rough | --no-rough)
      rough="on"
      [[ "$1" == "--no-rough" ]] && rough="off"
      shift
      ;;
    --close | --no-close)
      close="on"
      [[ "$1" == "--no-close" ]] && close="off"
      shift
      ;;
    *)
...
function make_affine_pyramid {
  ...
  while [[ $# -gt 0 ]]; do
    ...
    --linear-metric)
      ...
      ;;
    --linear-shrink-factors)
      linear_shrink_factors="$2"
      shift 2
      ;;
    --linear-smoothing-sigmas)
      linear_smoothing_sigmas="$2"
      shift 2
      ;;
    --linear-convergence)
  ...
  if [[ -n "${linear_shrink_factors:-}" && -n "${linear_smoothing_sigmas:-}" && -n "${linear_convergence:-}" ]]; then
    ...
    if [[ "${reg_type}" == "rigid" || "${reg_type}" == "lsq6" ]]; then
      ...
      single_transform='Similarity[ 0.1 ]'
    else
...
for initxfm in "${_arg_initial_transform[@]}"; do
  if [[ ${initxfm} == "com-masks" ]]; then
    ...
    antsAI --dimensionality 3 --verbose 0 \
      ...
      --transform Rigid[ 0.1 ] --search-factor [ 20,0.12 ] --align-principal-axes 0 \
      --convergence [ 10,1e-6,10 ] --translation-search-grid [ 40,0x0x0 ] \
      ${antsai_masks} ${_random_seed_arg} --output ${tmpdir}/antsai_init.mat
    initial_transform+="--initial-moving-transform ${tmpdir}/antsai_init.mat "
  elif [[ -s ${initxfm} ]]; then
...
if ! awk -v v="${_arg_otsu_foreground_threshold}" \
  'BEGIN{ exit !(v ~ /^[0-9]+([.][0-9]+)?$/ && v + 0 > 0 && v + 0 < 5) }'; then
  failure "--otsu-foreground-threshold must be a number larger than 0 and smaller than 5, got ${_arg_otsu_foreground_threshold}"
fi
...
fixed_object_extent=$(object_extent "${fixedfile1}" "${_arg_fixed_mask}" "${_arg_otsu_foreground_threshold}")
moving_object_extent=$(object_extent "${movingfile1}" "${_arg_moving_mask}" "${_arg_otsu_foreground_threshold}")
